...
- [x] 424 tests: those needing a database server skip unless one is reachable
...
- [x] Long-running soak test (`tests/ActorNet.Soak`): two nodes, a churning working set, and a
      verdict on heap, activations, dead letters, failure rate and throughput drift
...
- [x] API reference generated from the XML docs (`tools/ActorNet.ApiDocs` -> `docs/en/api/`),
      18 namespaces, 149 types - no DocFX, just the XML the compiler already writes
...
- **The API reference is English only, and generated by hand.** The Indonesian docs link to it
  rather than duplicating it: a generated reference translated once is a reference that is wrong
  after the next `///` edit. Nothing regenerates it automatically either, so it is as current as the
  last person who ran the generator.
