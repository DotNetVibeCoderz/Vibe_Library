...
over between nodes, a departing node hands its actors to their successors, and both halves of a
frame are binary between nodes; what is left is below, with the reasoning rather than only the
intent.
